...
let URL = "https://teachablemachine.withgoogle.com/models/DLd5JyHnF/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if ((time >= 5.67 && time <= 6.37 && !poseState.firstWindowTriggered) ||
                    (time >= 30.09 && time <= 30.79 && !poseState.secondWindowTriggered)) {
                    if (time <= 6.37) {
